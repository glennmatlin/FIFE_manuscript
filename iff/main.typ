...
A core capability of modern Language Models (LMs) is _instruction-following_—the ability to understand and execute a user's request precisely @he_can_2023. This skill, honed through techniques like instruction tuning @wang_self-instruct_2022 and Reinforcement Learning from Human Feedback (RLHF), has been pivotal in transforming LMs into helpful, general-purpose assistants. The success of models like InstructGPT demonstrated that even smaller models, when properly aligned, could outperform much larger base models in following user prompts, enhancing not only user satisfaction but also the truthfulness and safety of their outputs.
