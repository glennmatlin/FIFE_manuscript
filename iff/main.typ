...
#show: neurips2025.with(
  ...
  abstract: [
       Language Models (LMs) demonstrate an impressive ability to follow prompting instructions to guide their generation. However, the risk associated with hallucinations and confabulations when following complex and interdependent instructions has limited their effectiveness in high-stakes domains (i.e., finance), where precision and reliability are critical. We introduce `IFF`, a high-difficulty benchmark designed to assess the advanced instruction-following capabilities of LMs for finance. We provide #num_prompts human-authored prompts that mirror complex financial analysis tasks, utilizing table information extraction, named entity recognition, and question answering. We construct a _verification system_ of instruction constraints that can be chained together in a complex manner. Each instruction uses automated verification to provide fine-grained reward signals. We evaluate #num_models_evaluated models in a zero-shot setting, including the most popular proprietary models (GPT5, Claude Opus 4.1), open-weight models (Qwen3, Llama4), and open-source models (OLMo2). Our key findings reveal a clear hierarchy in instruction-following: open-weight models now lead the benchmark, surpassing even the strongest proprietary systems, while fully open-source models continue to lag behind. Surprisingly, models explicitly tuned for complex reasoning do not exhibit stronger reliability, and in some cases underperform general-purpose models. Across all categories, failures often arise from precise formatting or structural requirements, with the gap between strict and loose metrics showing that many outputs are “near misses” rather than fundamentally incorrect. We release the benchmark dataset and code as an open resource to foster research into Reinforcement Learning with Verified Rewards (RLVR) for language models in the financial domain.
  ],
