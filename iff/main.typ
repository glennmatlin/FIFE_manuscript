...
#show: neurips2025.with(
  title: [Instruction Following for Finance:  Verifying language models' ability to follow 
  complex financial instructions],
  ...
  abstract: [
       ...
       .#footnote[The code and data are available for review at: https://anonymous.4open.science/r/iff_anon-443F/].
  ],
...
Our evaluation has two primary limitations. First, we rely on a single response per prompt due to API cost, which limits robustness against stochastic model variation. Second, our regex-based validators, while transparent and reproducible, can be brittle. They may occasionally fail to recognize a semantically correct response due to minor formatting deviations. More flexible validation methods, such as LM-as-a-judge or hybrid semantic checks, are a natural next step for future work.

// A key limitation of this study is that our evaluation relies on a single response sample for each task from every model. The significant API costs associated with generating multiple responses per prompt were prohibitive, given our team’s limited funding. Evaluation strategies using multiple inference samples would improve reliability and mitigate the potential impact of stochasticity in model outputs.
// Another limitation stems from our reliance on regex-based validators, which can be brittle in distinguishing genuinely correct responses from those that only deviate in formatting or phrasing. While these rules provide transparent and reproducible checks, they can penalize models unevenly depending on alignment with validation patterns. More robust alternatives,such as leveraging LLMs themselves as evaluators (“LLM-as-a-judge”) or combining regex with semantic similarity measures could substantially improve evaluation fidelity. We view such hybrid approaches as an important direction for future iterations of the benchmark.
...
// This study introduced the IFF Benchmark to evaluate instruction-following reliability across closed-source, open-source, and open-weight large language models. Our results demonstrate a clear stratification: open-weight models now set the frontier, closed-source systems remain strong but no longer dominant, and fully open-source efforts continue to lag behind. The success of open-weight releases underscores the growing competitiveness of publicly accessible models, while the consistently high performance of closed-source systems highlights their stability. At the same time, our evaluation reveals important gaps. Reasoning-oriented models, despite being explicitly designed for multi-step problem solving, do not translate this advantage into superior instruction adherence, suggesting that advances in reasoning or scaling alone do not guarantee robustness. The gap between strict and loose metrics further indicates that many failures are near-misses, where models produce “almost correct” responses but miss critical formatting or structural details.
// Taken together, these findings emphasize that progress in instruction following is uneven. While leading open-weight systems rival or surpass the best proprietary models, fully open-source approaches remain limited, and reasoning capabilities do not automatically ensure compliance with fine-grained instructions. Future research will benefit from addressing these blind spots and from developing evaluation methods that probe both correctness and reliability under diverse conditions.
